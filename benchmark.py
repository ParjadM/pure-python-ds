...
def run_benchmark():
    ...
    print("|---|---|---|---|")
    print(f"| Insert {N} items | {list_insert_time:.4f} sec | {avl_insert_time:.4f} sec | List: $O(1)$ / AVL: $O(\\log n)$ |")
    print(f"| Search {SEARCH_COUNT} items | {list_search_time:.4f} sec | {avl_search_time:.4f} sec | List: $O(n)$ / AVL: $O(\\log n)$ |")
