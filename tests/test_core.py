import pytest
from pure_python_ds.linear import Stack, Queue, MinHeap, MaxHeap, HashTable, DoublyLinkedList, SinglyLinkedList
...
def test_linear_master():
    ...
    s.push(10); assert len(s) == 1
    
    q = Queue[int]()
    assert q.dequeue() is None
    q.enqueue(10); assert len(q) == 1

    ht = HashTable(size=2)
    for i in range(15): ht.set(f"key_{i}", i)
    assert ht.get("key_14") == 14
    assert ht.get("miss") is None

    sll = SinglyLinkedList[int]()
    sll.append(5); sll.remove(5)
    assert sll.search(999) is False
    assert sll.remove(99) is False

    dll = DoublyLinkedList[int]()
    for x in [10, 20, 30]: dll.append(x)
    dll.remove(20); dll.remove(30); dll.remove(10)
...
def test_trees_master():
    ...
    for x in range(1, 15): rbt.insert(x)
    assert rbt.root is not None

    t = Trie()
    t.insert("apple"); t.insert("apply"); t.delete("apple")
    ...
    binary_t.insert(5); binary_t.insert(15)
    ...
    for x in [50, 30, 70, 60, 80]: bst.insert(x)
    bst.delete(70)
    assert bst.search(70) is None

def test_avl_exhaustive_deletion():
    avl = AVLTree[int]()
    for n in [50, 30, 70, 20, 40, 60, 80, 10]: avl.insert(n)
    avl.delete(999) # Miss
...
def test_complex_tree_rotations():
    ...
    bt.insert(5); bt.insert(15)
    if hasattr(bt, 'preorder'): list(bt.preorder())
    if hasattr(bt, 'postorder'): list(bt.postorder())
...
def test_avl_deletion_rotations_explicit():
    ...
    avl_ll = AVLTree[int]()
    for x in [30, 20, 40, 10]: avl_ll.insert(x)
    ...
    avl_rr = AVLTree[int]()
    for x in [30, 20, 40, 50]: avl_rr.insert(x)
